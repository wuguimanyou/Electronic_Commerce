...
<script type="text/javascript">
    ...
    function gotoViewRerecordDetail(index){
      var id = index;
			window.location.href="my_hongbao_log.php?customer_id="+customer_id_en+"&id="+id;
	}   
	hongbaoList();
		 ...
		 $.ajax({
				...
				success:function(res){
					...
					for(id in res){
						...
						content += '		<img class="" src="./images/info_image/hongbao_1.png" />';
						content += '	</div>';
						...
						content += '		</div>';
						content += '		<div class="info_right"><span style="color:black;">'+res[id]['red_money']+'元</span></div>';
						content += '	</div>';
						content += '</div>';
					}
					$('.red').html(content);
