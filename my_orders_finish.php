...
$query = "select status,paystatus,sendstatus,address_id from weixin_commonshop_orders where isvalid=true and customer_id=".$customer_id." and user_id=".$user_id." and batchcode=".$batchcode." limit 1";
	...
	<div class="mainArea">
		<div class="entry-content">
			...
			<?php
						...
						$dcount = 0;
						$dcount1 = 0;
						$query2 = "select count(1) as dcount from weixin_commonshop_orders where isvalid=true and customer_id=".$customer_id." and user_id=".$user_id." and batchcode=".$batchcode." and is_discuss=0";
						$result2 = mysql_query($query2) or die('query failed2'.mysql_error());
						while($row2 = mysql_fetch_object($result2)){
							$dcount = $row2->dcount;
						}
						if($dcount==0){
							$query2 = "select count(1) as dcount from weixin_commonshop_orders where isvalid=true and customer_id=".$customer_id." and user_id=".$user_id." and batchcode=".$batchcode." and is_discuss=1";
							$result2 = mysql_query($query2) or die('query failed2'.mysql_error());
							while($row2 = mysql_fetch_object($result2)){
								$dcount1 = $row2->dcount;
							}
						...
						/* 判断订单是否已评价 end*/
						/* 店铺 start*/
						$sql = "select pid,createtime,paytime,confirm_sendtime,confirm_receivetime from weixin_commonshop_orders where isvalid=true and customer_id=".$customer_id." and user_id=".$user_id." and batchcode=".$batchcode." limit 1";
						...
						while($row_s = mysql_fetch_object($res)){
							...
							$confirm_receivetime = $row_s->confirm_receivetime;	//确认收货时间
						}
				...
				<li class="itemWrapper item_goods">
					<div class="itemMainDiv">
							...
							<div class="contentLiDiv">            
								...
								<div class="itemProContent right_white_rect" onclick="clickedShenQingShouHou(<?php echo $pid;?>,<?php echo $batchcode;?>);">申请售后</div>								
							</div>
					</div>
				</li>
			...
			<div class="infoWrapper">
				...
				<span class="content-line" style="margin-bottom:10px;">成交时间:<?php echo $confirm_receivetime;?></span>
			</div>
		...
		<div style="width:100%;">
			<ul class="am-navbar-nav am-cf am-avg-sm-1">
				...
				<?php
					if($dcount>0){
				?>
					<span onclick="clickedPingjia(<?php echo $batchcode;?>);" class="am-navbar-label btnWhite2">评价</span>
				<?php
					}else if($dcount1>0){
				?>
					<span onclick="clickedPingjia(<?php echo $batchcode;?>);" class="am-navbar-label btnWhite2">追加评价</span>
				<?
					}
				?>
					<span onclick="clickedChaKanWuLiu(<?php echo $batchcode;?>);" class="am-navbar-label btnWhite4">查看物流</span>
				</li>
			 </ul>
		</div>
    </div>
...
<script type="text/javascript">
	...
	//点击【申请售后】
	function clickedShenQingShouHou(productID,orderID){
		window.location.href = "aftersale.php?pid="+productID+"&batchcode="+orderID;
	}
	...
	//点击【评价】
	function clickedPingjia(dingdanID){
