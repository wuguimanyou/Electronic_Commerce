<?php  
	...
	$query_order = "
		SELECT 
		orders.pay_batchcode,orders.id,orders.user_id,orders.batchcode,orders.createtime,orders.paystyle,sum(orders.totalprice) as  totalprice,orders.paystatus,orders.sendstatus,orders.status,orders.exp_user_id,orders.supply_id,orders.allipay_orderid,orders.is_delay,orders.store_id,
		orders.return_type,orders.confirm_sendtime,orders.confirm_receivetime,orders.sendtime,orders.printUrl,orders.paytime,orders.agent_id,orders.Pay_Method,
		...
		orders.aftersale_type,orders.aftersale_state,orders.return_status,orders.return_account,orders.is_QR,orders.expressnum,orders.expressname,
		orders.sendstyle,orders.backgoods_reason,orders.store_id,orders.store_name,orders.isreducesupply, 
		users.name,users.weixin_name,users.weixin_fromuser,users.phone,
		addr.name as expressName,addr.phone as expressPhone,addr.location_p,addr.location_c,addr.location_a,addr.address as expressAddress,addr.identity,addr.identityimgt,addr.identityimgf
		...
		file_put_contents("aww.txt",date("Y-m-d")."====query_orders".var_export($query_order,true)."\r\n",FILE_APPEND);
	...
	if(!empty($_GET["search_supply_id"])){ 	//订单供应商ID
	...
	//订单所属
	   $search_order_ascription = $configutil->splash_new($_GET["search_order_ascription"]);	
	    switch($search_order_ascription){
			...
			default:
				//供应商订单
				if($search_supply_id != $search_order_ascription && $search_supply_id != -1 && !empty($search_supply_id) ){
					$search_order_ascription = $search_supply_id;
        ...
        <div style="width:100%;display:block;">
			<ul>
         		<li class="WSY_bottonli WSY_bottonli2" style="display:block;float:left;margin-left:20px;">
					...
					<input type="button"  value="一键完成订单" onClick="Onekey_Complete_all_orders();" >
				 </li>  
			</ul>
		...
		<div class="WSY_position1" >
			<ul>		
				<li class="WSY_position_text">
					...
					<a>供应商ID：<input type="text" name="search_supply_id" id="search_supply_id" value="<?php echo $search_supply_id; ?>" onkeyup="this.value=this.value.replace(/[^\d]/g,'');"></a>
		...
		<table width="97%" class="CP_table " id="order_table">
			 ...
			 //echo $query_order;
				...
				while ($row_order = mysql_fetch_object($result_order)) {
					...
					$result_in = mysql_query($sql_in) or die('Query_express failed: ' . mysql_error());
					$invoice_head = "";
					...
					if($o_express_id>0){
						...
						$result_expressname=mysql_query($query_expressname) or die ('query_expressname faild' .mysql_error());
						while($row=mysql_fetch_object($result_expressname)){
							$new_expressname=$row->name;						
						}
					}
					$query_change="select price,CouponPrice,SupplyCostMoney,ExpressPrice from weixin_commonshop_order_prices where isvalid=true and batchcode='".$o_batchcode."' limit 1";					
					...
					$result_cashback = mysql_query($query_cashback) or die('query_cashback failed: ' . mysql_error());
					$cashback_rows = mysql_num_rows($result_cashback);
					if($cashback_rows < 1){
						$query_cashback_t = "select id from cashback_t where batchcode='".$o_batchcode."'";
						$result_cashback_t = mysql_query($query_cashback_t) or die('query_cashback_t failed: ' . mysql_error());
					...
					$result = mysql_query($query) or die('Query_weipay_currency failed: ' . mysql_error());
			 ...
			 <tr class="CP_table_bianhao" >
				...
				<td class="CP_table_bianhaoa" colspan="8">
					...
					<img style="width:18px;height:18px;margin-left:2px" src="../../../common/images_V6.0/contenticon/gong.png" ondragstart="return false;" title="供应商订单" />
					<?php }  if($o_is_QR==1){   ?>
					...
					<img style="width:18px;height:18px;margin-left:2px" src="../../../common/images_V6.0/contenticon/cashback.png" ondragstart="return false;" title="返现订单" />
					<?php }?>					
				...
				if($o_paystatus==1 and $o_Pay_Method==0){ 
					...
					if($o_paystyle=="通联支付"){ 
					...
					}elseif($o_paystyle=="微信支付" or $o_paystyle=="找人代付" ){
						/* 微信支付 */
						...
						$paySql = "select price from paycallback_t where isvalid=true and pay_batchcode='".$pay_batchcode."' limit 0,1";
						$result_paySql = mysql_query($paySql) or die('paySql failed: ' . mysql_error());
			 ...
			 <?php 
					/* 产品信息 */
						...
						$o2_imgurl          = 0; 
						$o2_cost_price         = 0;//供货价 
						$query_order2 = "SELECT orders2.rcount,orders2.prvalues,orders2.prvalues_name,orders2.totalprice,orders2.isvalid,orders2.merchant_remark, 
							product.name,product.foreign_mark,product.default_imgurl,product.id,product.cost_price
						...
						while ($row_order2 = mysql_fetch_object($result_order2)) { 
							...
							/* 产品图片 */
							if(empty($o2_imgurl)){
								$query_imgurl="select imgurl from weixin_commonshop_product_imgs where  product_id=".$o2_pid." limit 0,1";
							...
							}else  */if( !empty($o2_prvalue) /* and empty($o2_prvalues_name) */ ){
					...
					$query_op = "SELECT is_Pinformation from weixin_commonshops_extend where isvalid=true and customer_id=".$customer_id;
						$result_op = mysql_query($query_op) or die('Query_op failed: ' . mysql_error());
			 ...
			 <tr class="CP_table_chanpina">	
				...
				<td class="CP_table_chanpina_one">
					...
					<span class="CP_table_chanpina_onep">
						...
						$result_m = mysql_query($query_m) or die('Query_mess failed: ' . mysql_error());
						$row_m = mysql_fetch_object($result_m);
						$row_m = $row_m ->num;
						if( 0 < $row_m and $is_Pinformation == 1 ){
						?>
						<div>
							<p><b>必填信息</b></p>
							<div class="mess_b" style="max-height: 48px;overflow: hidden;"><!-- 必填信息框架 -->
								...
								$result_mess = mysql_query($query_mess) or die('Query_mess failed: ' . mysql_error());
				...
				if($o2_isvalid==1){
					...
					<a id="button_print_<?php echo $o_batchcode; ?>" title="打印快递运单" class="print_delivery" style="display:none;"  onclick="showExpress('<?php echo $o_batchcode; ?>')" ><img src="../../../common/images_V6.0/operating_icon/icon29.png" /></a>
					...
					<?php if($o_aftersale_state==4 && $o_supply_id>0 && !$o_isreducesupply){?>
						<a title="扣除供应商款项" class="reducesupply_btn" data-reduce-supply-batchcode="<?php echo $o_batchcode;?>"  onclick="reducesupply('<?php echo $o_batchcode;?>')" ><img src="../../../common/images_V6.0/operating_icon/icon57.png" /></a>					
					<?php }?>
				...
				<?php if($cashback_rows>0){?>									
				<a title="返现记录" href="order_cashback_log.php?batchcode=<?php echo $o_batchcode; ?>&customer_id=<?php echo passport_encrypt($customer_id)?>"><img src="../../../common/images_V6.0/operating_icon/icon72.png" /></a>
				<?php }?>
          ...
          <tr class="WSY_positiontrhide">
          	<td colspan="11" class="order_td">
				...
				<!--订单详情End-->
				<div class="order order_delivery_dl order_hide div_show" id="print_express_<?php echo $o_batchcode; ?>"  >
					<i class="guanbi" onclick="hideDetail()" ><img class="WSY_modifypimg" src="../../../common/images_V6.0/contenticon/gbicon.png" alt=""></i><!--点击关闭信息-->
                    <dl>
                        <form>
                        ...
                        <div class="order_div01">
                            ...
                            <dd><b>选择快递模板：</b><span>
                                <select id="print_express_id_<?php echo $o_batchcode; ?>">
                                   							
										   <?php 
										   //$sf_id = -1;
										   $query_print_sql = 'SELECT id,name FROM weixin_expresses where isvalid=1 and customer_id='.$customer_id;
										   
										   $result_print_express = mysql_query($query_print_sql) or die('Query_express failed: ' . mysql_error()); 
										   while ($row_express = mysql_fetch_object($result_print_express)) {
											  $express_id= $row_express->id;
											  $express_name = $row_express->name;
										   ?>
											<option value="<?php echo $express_id; ?>"  ><?php echo $express_name; ?></option>
										   <?php } ?>	
                        ...
                        <dd class="WSY_bottonli con-button"><input  type="button" value="确定打印" onclick="print_delivery('<?php echo $o_batchcode; ?>')"   ></dd>
                            ...
                            <dd><b>物流公司：</b><span>
                                <select id="express_id_<?php echo $o_batchcode; ?>">
										   ...
										   if($o_supply_id>0){ 
												...
												$query_express = 'SELECT id,expresses_name FROM weixin_expresses_company where isvalid=true and customer_id='.$customer_id." order by is_default desc";   
										   }
										   $result_express = mysql_query($query_express) or die('Query_express failed: ' . mysql_error()); 
										   while ($row_express = mysql_fetch_object($result_express)) {
											  $e_id= $row_express->id;
            	...
            	<div class="WSY_modifydiv order_hide div_show" id="refund_<?php echo $o_batchcode; ?>" > 
                    <dl class="order_date">
						...
						<dd>
							...
							<span style="font-size:12px;width:100%;text-align:left;color:#f00;">(最多结算給供应商 <?php echo $o_SupplyCostMoney+$o_SupplyExpressPrice;?>元)</span>
						</dd>
						<?php }?>
						<dd>
							<b style="width:145px;">实际退款金额：</b>
							<span>
								<input type="text" onkeyup="clearNoNum(this)" id="good_refund_<?php echo $o_batchcode;?>" value="<?php echo $row_order->return_account;?>">
							</span>
							<b style="width:40px;">元</b>
						</dd>
						<?php
						...
						<dd style="text-align: right;margin-right: 96px;">
						+
						</dd>
						<?php
						}
						...
						<dd>
							...
							<b style="width:40px;">元</b>
							<span style="font-size:12px;width:100%;color:#f00;">(该订单使用了 <?php echo $settlementcurrency;?>购物币)</span>
						</dd>
			  ...
			  <!--修改价格·定位属性-->
            	...
            	<div class="WSY_modifydiv order_hide" id="reduce_<?php echo $o_batchcode; ?>" > 
                    <dl class="order_date">
                       <dt><a style="width:110px;">扣除供应商款项</a></dt>
						...
						<dd>
							...
							<b style="width:40px;">元</b>
							<span style="font-size:12px;width:100%;color:#f00;">(最多扣除供应商 <?php echo $o_SupplyCostMoney+$o_SupplyExpressPrice;?>元)</span>
						</dd>
                        ...
                        <dt><a>订单日志</a></dt>
							...
							<?php 
									...
									switch($op){
										...
										case 23 :$op_str = "维权扣除供应商款项";break;
									}
									echo $op_str;
								?>								
								</span>
...
function showExpress(batchcode){
	var div = $("#print_express_"+batchcode);
	$(".div_show").not(div).hide();
	if(div.is(":hidden")){
		div.fadeIn("slow");
	}else{
...
$(".good_refund").click(function(){	
	...
	layer.confirm('订单:'+batchcode+'</br>将退款 <b style="color:red">'+total_refundMoney+'</b> 元</br><b style="color:red">微信支付订单请先到微信支付详情界面进行手动退款！</b>', {
		btn: ['退款','取消'] 
	}, function(confirm){
...
$(".order_reduce").click(function(){	
	...
	layer.confirm('您确认要对 订单:'+batchcode+' </br>的供应商扣除金额 '+reducemoney+'吗', {
		btn: ['扣除','取消'] 
	}, function(confirm){
...
function print_delivery(batchcode){
	var template_id = $("#print_express_id_"+batchcode).val();
	//alert(<?php echo passport_decrypt($customer_id); ?>);
	print3order(batchcode,template_id,<?php echo passport_decrypt($customer_id); ?>,1);
}
...
//2016-8-30黄
function Onekey_Complete_all_orders(){
layer.confirm('您确定要完成所有确认收货的订单吗？', {//提示
		title:'一键完成订单',		//标题
		btn: ['确定','取消'] //按钮确定，取消
	}, function(){
		$.ajax({
			url: "Onekey_Complete_all_orders.php",//提交的页面
			type:"POST",//post方法
			data:{'op':"finish_order"},//传参
			dataType:"json",//返回 JSON 数据
			success: function(res){//提交成功后的回调,返回msg的内容
				
				layer.alert(res.msg);//提示订单完成成功
			},	
			error:function(){
				layer.alert("网络错误请检查网络");//错误返回的提示
			}						
		});			
	}, function(){
		layer.msg('已取消', {
