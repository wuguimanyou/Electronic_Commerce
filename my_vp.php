...
<style>  
   ...
   .info_middle_content{width:45%;float:left;}
   ...
   .info_middle{width:94%;height:70px;background-color:white;margin:0 auto;border-bottom:1px solid #eee;}
...
<script type="text/javascript">
    ...
    function gotoViewRerecordDetail(index){
      var id = index;
			window.location.href="my_vp_log.php?customer_id="+customer_id_en+"&id="+id;
	}   
	vpList(1);
	function vpList(page){
		 var from = 'vp';
		 $.ajax({
				...
				success:function(res){
					...
					for(id in res){
						...
						content += '		</div>';
						content += '		<div class="info_right"><span style="color:black;">vp值：'+res[id]['vp']+'</span></div>';
						content += '	</div>';
						content += '</div>';
					}
					$('.vp').append(content);
